...
public:
	...
	uint8_t *getExtraData() const { return extraData; }
	...
	bool loadDDS(const char *fileName, uint32_t flags = 0);

	bool saveHTEX(const char *fileName);
	bool saveDDS(const char *fileName);

	bool loadImage(const char *fileName, uint32_t flags = 0);
	bool loadSlicedImage(const char **fileNames, const int nImages, const int nArraySlices = 1, uint32_t flags = 0);
