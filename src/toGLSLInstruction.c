...
void AddAssignToDest(HLSLCrossCompilerContext* psContext, Operand* psDest)
{
	bstring glsl = *psContext->currentGLSLString;
	SHADER_VARIABLE_TYPE eDestDataType = GetOperandDataType(psContext,psDest);
	if(eDestDataType == SVT_INT)
 ...
 int dest, int src0, int src1, uint32_t dataType)
{
	...
	if(src1SwizCount == src0SwizCount == dstSwizCount)
	{
		...
		AddAssignToDest(psContext, &psInst->asOperands[dest]);
		bcatcstr(glsl, "(");
    ...
    uint32_t ui32Flags)
{
    ...
    else
    {
        ...
        if(ui32Flags & (TEXSMP_FLAG_LOD|TEXSMP_FLAG_FIRSTLOD))
        {
            bformata(glsl, " = floatBitsToInt(%sLod%s(", funcName, offset);
        }
        else
        if(ui32Flags & TEXSMP_FLAGS_GRAD)
        {
             bformata(glsl, " = floatBitsToInt(%sGrad%s(", funcName, offset);
        }
        else
        {
            bformata(glsl, " = floatBitsToInt(%s%s(", funcName, offset);
        }
...
void SetDataTypes(HLSLCrossCompilerContext* psContext, Instruction* psInst, const int32_t i32InstCount)
{
	...
	SHADER_VARIABLE_TYPE eNewType = SVT_INT;
	...
	for(i=0; i < i32InstCount; ++i, psInst++)
	{
		...
		case OPCODE_MOVC:
		{
			...
			if(psOperand->eType == OPERAND_TYPE_TEMP)
			{
				eNewType = SVT_INT;//GetOperandDataType(psContext, &psInst->asOperands[2]);
...
void TranslateInstruction(HLSLCrossCompilerContext* psContext, Instruction* psInst)
{
    ...
    switch(psInst->eOpcode)
    {
        ...
        case OPCODE_DP3:
        {
			...
			AddAssignToDest(psContext, &psInst->asOperands[0]);
        ...
        case OPCODE_MOVC:
        {
			...
			const SHADER_VARIABLE_TYPE eSrcAType = GetOperandDataType(psContext, &psInst->asOperands[1]);
			const SHADER_VARIABLE_TYPE eSrcBType = GetOperandDataType(psContext, &psInst->asOperands[2]);
			const SHADER_VARIABLE_TYPE eSrcCType = GetOperandDataType(psContext, &psInst->asOperands[3]);
#ifdef _DEBUG
            AddIndentation(psContext);
            bcatcstr(glsl, "//MOVC\n");
#endif
...
*/
			...
			if(s0ElemCount==1 || IsSwizzleReplacated(&psInst->asOperands[1]))
			{
				...
				{
					bcatcstr(glsl, " ivec4( ");
					//bformata(glsl, "%s4(", GetConstructorForType(eSrcBType, 4));
				}
                TranslateOperand(psContext, &psInst->asOperands[2], ui32Flags);
				if(destElemCount != s1ElemCount)
				{
					bcatcstr(glsl, ")");
				...
				{
					bcatcstr(glsl, " ivec4( ");
					//bformata(glsl, "%s4(", GetConstructorForType(eSrcCType, 4));
				}
                TranslateOperand(psContext, &psInst->asOperands[3], ui32Flags);
				if(destElemCount != s2ElemCount)
				{
					bcatcstr(glsl, ")");
					AddSwizzleUsingElementCount(psContext, destElemCount);
            ...
            for(destElem=0; destElem < destElemCount; ++destElem)
            {
                ...
                AddIndentation(psContext);
                bcatcstr(glsl, "if(");
                TranslateOperand(psContext, &psInst->asOperands[1], ui32Flags);
                if(s0ElemCount>1)
